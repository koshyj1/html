...
export class ApiService {
    ...
    public async updateVetLogo(logo: File) : Promise<File> { // Promise<void>
        // const uploadData = new FormData();
        // uploadData.append('image', logo, logo.name);
        // // uploadData.find(logo);
        console.log("ready");

        // return this.http.put<void>(environment.baseUrl + '/api/settings/vet-settings/logo/', uploadData).toPromise();
        return this.http.get<File>(environment.baseUrl + `/api/settings/vet-settings/logo/${logo}`).toPromise();
    }
